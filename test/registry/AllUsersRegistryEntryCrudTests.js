...
contract("All Users Registry Entry Crud Tests", (accounts) => {
    ...
    before(async () => {
        registry = await RegistryUtils.createTestRegistry(
            ...
            RegistryUtils.RegistryCreateEntryPermissionGroup.AllUsers
        )
    })

    /*  -------------------------------- Create Entry -----------------------  */
    //todo discuss: value can be only equal defined fee
    it("#1 should allow registry admin to add new entry with enough fee", async () => {
    ...
    it("#2 should allow registry owner to add new entry with enough fee", async () => {
    ...
    it("#3 should allow unknown to add new entry with enough fee", async () => {
    ...
    it("#4 should not allow admin to add new entry without enough fee", async () => {
        await registry.createEntryPromise(REGISTRY_ADMIN_ACCOUNT, registry.fee - 1).should.be.rejected
    })

    it("#5 should not allow unknown to add new entry without enough fee", async () => {
        await registry.createEntryPromise(UNKNOWN_ACCOUNT, registry.fee - 1).should.be.rejected
    })

    /*  -------------------------------- Delete Entry -----------------------  */
    it("#6 should allow registry admin to delete entry if her is entry owner", async () => {
    ...
    it("#7 should allow registry owner to delete entry if her is entry owner", async () => {
    ...
    it("#8 should not allow registry admin to delete entry if her isn't entry owner", async () => {
    ...
    it("#9 should not allow registry owner to delete entry if her isn't entry owner", async () => {
        const newEntryId = await registry.createEntry(UNKNOWN_ACCOUNT)
        await registry.containsEntry(newEntryId).should.eventually.equal(true)
    ...
    it("#10 should allow unknown to delete entry if her is entry owner", async () => {
        const newEntryId = await registry.createEntry(UNKNOWN_ACCOUNT)
        await registry.containsEntry(newEntryId).should.eventually.equal(true)
    ...
    it("#11 should not allow unknown to delete entry if her isn't entry owner", async () => {
        const newEntryId = await registry.createEntry(REGISTRY_ADMIN_ACCOUNT)
        await registry.containsEntry(newEntryId).should.eventually.equal(true)
